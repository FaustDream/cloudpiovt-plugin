...
绝对路径历史、选择应用、一键打开编辑器依赖原生助手。发布包已内置自包含 `CloudPiOvt.NativeHost.exe`，普通用户不需要安装 .NET SDK / Runtime。
...
```powershell
pwsh .\scripts\package-extension-release.ps1 -Version 1.1.0
```

该脚本会先生成 win-x64 自包含原生助手运行目录，再输出包含扩展和安装入口的 zip 包。
...
正常状态显示：“原生助手已连接”。如果显示“原生助手未安装”，请双击 `scripts\install-native-host.cmd` 后重新打开设置页。
...
普通用户不需要处理扩展 ID。发布包构建时会把 manifest key 推导出的扩展 ID 写入 `allowed_origins`；Edge 若生成不同 ID，需要开发维护人员在构建时追加当前 Edge 扩展 ID。

### 8.2 重新注册

如果 Edge 中显示“原生助手未安装”，通常是发布包未包含当前 Edge 扩展 ID。开发维护人员重新构建时追加 `-ExtensionId <Edge扩展ID>`。
...
原因：浏览器目录权限未授予或已失效。点击「目录句柄」旁边的刷新按钮重新选择目标目录。
...
原因：尚未安装原生助手，或当前浏览器扩展 ID 未写入 `allowed_origins`。先双击 `scripts\install-native-host.cmd`；Edge 仍失败时，让开发维护人员用当前 Edge 扩展 ID 重新打包。
