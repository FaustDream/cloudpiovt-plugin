...
/**
 * 仅按域名判断当前页面是否属于已知平台（氚云 h3yun.com / 云枢 ztna-dingtalk.com）。
 * 与 resolvePlatformKey 使用同一组域名，作为“是否已明确识别平台”的权威依据：
 * 命中即视为已识别（含首页/门户，不限于具体设计页），未命中（含 google、chrome:// 等）视为 unknown。
 */
export function isRecognizedPlatformUrl(pageUrl) {
  try {
    const hostname = new URL(pageUrl || "").hostname.toLowerCase();
