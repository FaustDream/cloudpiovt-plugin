...
export function normalizeCustomLaunchers(value) {
  ...
  const customLaunchers = inputLaunchers
    .filter((launcher) => {
      // 排除仍在 BUILTIN_LAUNCHER_BY_ID 中的内置条目（它们已在 builtins 中处理）
      if (BUILTIN_LAUNCHER_BY_ID.has(String(launcher?.launcherId || ""))) {
        return false;
      }
      // 排除已从 BUILTIN_LAUNCHERS 中移除的历史内置条目（如 builtin-git-bash）
      // 这些孤儿数据标记为 builtin:true 但不再有对应的内置定义
      if (launcher?.builtin === true) {
        return false;
      }
      return true;
    })
