...
export async function statNativeDirectoryFiles({ directoryPath, fileNames }) {
  return sendNativeMessage({
    command: "stat_directory_files",
    directoryPath: String(directoryPath || ""),
    files: Array.isArray(fileNames)
      ? fileNames.map((fileName) => ({
          fileName: String(fileName || "")
...
export async function discoverNativeLaunchers() {
  return sendNativeMessage({
    command: "discover_launchers"
  });
}

export async function extractExecutableIcon({ executablePath, iconKey }) {
  return sendNativeMessage({
    command: "extract_executable_icon",
    executablePath: String(executablePath || ""),
    iconKey: String(iconKey || "")
  });
}

export async function saveLauncherIcon({ iconKey, fileName, content }) {
  return sendNativeMessage({
    command: "save_launcher_icon",
    iconKey: String(iconKey || ""),
    fileName: String(fileName || ""),
...
export async function deleteLauncherIcon({ iconKey }) {
  return sendNativeMessage({
    command: "delete_launcher_icon",
    iconKey: String(iconKey || "")
  });
}
