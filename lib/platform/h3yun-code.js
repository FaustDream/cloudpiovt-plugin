...
export function resolveH3yunBackendFileName(input = {}) {
  ...
  // 列表设计模式下追加 _ListViewController 后缀，遵循 C# ListViewController 命名约定。
  const fileBaseName =
  ...
  const modeSuffix = input.designMode === "list" ? "_ListViewController" : "";
  const baseWithSuffix = `${fileBaseName}${modeSuffix}`;

  return baseWithSuffix.toLowerCase().endsWith(".cs") ? baseWithSuffix : `${baseWithSuffix}.cs`;
}

export function resolveH3yunFrontendFileName(input = {}) {
  // 氚云前端 JS 没有稳定类名，优先使用表单对象 ID，便于和同表单的 C# 文件配对。
  // 列表设计模式下追加 _ListViewController 后缀，与 C# 后端文件保持一致。
  const fileBaseName =
    sanitizeCSharpFileBaseName(extractH3yunDesignerId(input.pageUrl))
  ...
  return baseWithSuffix.toLowerCase().endsWith(".js") ? baseWithSuffix : `${baseWithSuffix}.js`;
}
...
export function buildH3yunFromCodeContent(metadata = {}) {
  ...
  for (const control of controls) {
    ...
    // 隐藏规则始终输出（即使为空也保留行），方便后续程序化解析处理
    lines.push(`隐藏规则: ${cleanInlineText(control.displayRule)}`);
